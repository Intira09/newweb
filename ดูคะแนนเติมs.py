...
<head>
  ...
  <title>ผลการตรวจคำตอบ</title>
    ...
    pre {
      ...
      border-radius: 4px;
      overflow-x: auto;
      overflow-y: auto;   /* ✅ เพิ่มเลื่อนแนวตั้ง */
...
<body>
  ...
  <div class="container">
    ...
    <!-- แสดงปีการศึกษาและชั้นเรียนแบบฟิก -->
    <div class="readonly-info">
      📘 <strong>ปีการศึกษา:</strong> <span id="displayYear"></span> |
      🏫 <strong>ระดับชั้น:</strong> <span id="displayGroup"></span>
    </div>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", async () => {
      if (!examYear || !examGrade) {
        alert("⚠️ ยังไม่ได้ตั้งค่าปีการศึกษาและระดับชั้น กรุณากลับไปที่หน้าตั้งค่าข้อสอบ");
      ...
      try {
        ...
        // ✅ mapping ชั้นเรียน
        const groupMap = {
          "ประถมศึกษาปีที่ 6": "P6",
          "มัธยมศึกษาปีที่ 3": "M3"
        };
        const mappedGrade = groupMap[examGrade] || examGrade;

        // ✅ กรองข้อมูลตามปีและชั้น
        allData = allData.filter(d => d.exam_year == examYear && d.group_id == mappedGrade);

        // ✅ โหลด dropdown ครั้งแรก
        updateStudentDropdown();

        // ✅ โหลดใหม่เมื่อเปลี่ยนสถานะ
        document.getElementById("statusFilter").addEventListener("change", updateStudentDropdown);
    ...
    function updateStudentDropdown() {
      ...
      if (statusFilter) {
        if (statusFilter === "ตรวจแล้ว") filtered = filtered.filter(d => d.status === "ตรวจแล้ว");
        else if (statusFilter === "ยังไม่ได้ตรวจ") filtered = filtered.filter(d => d.status !== "ตรวจแล้ว");
      }

      filtered.forEach(s => {
        const opt = document.createElement("option");
        opt.value = s.answer_id;
        opt.textContent = `${s.student_id} (${s.status})`;
        studentSelect.appendChild(opt);
      });

      console.log("🔍 นักเรียนที่โหลด:", filtered.length, "คน");
    }
    ...
    async function loadStudentData() {
      ...
      try {
        ...
        // 🔥 แก้ไขคะแนนที่ผิดพลาด (ดึงจาก feedback)
        Object.keys(parsedDescription).forEach(key => {
          ...
          if (item.score === 0 && item.feedback) {
            try {
              ...
              if (key === 's1' && feedbackObj['คะแนนรวม'] != null) {
                item.score = parseFloat(feedbackObj['คะแนนรวม']);
                console.log(`🔧 แก้ไข ${key}: 0 → ${item.score}`);
              }
              ...
              console.warn(`⚠️ ไม่สามารถ parse feedback ของ ${key}:`, e);
            }
          }
        });
        ...
        try {
          ...
          console.log("✅ Parsed teacherScores:", teacherScores);
        } catch (e) {
          console.error("❌ Error parsing teacher_scores:", e);
        }
        ...
        console.log("📊 คะแนนรวม AI:", totalAIScore);
        ...
        const criteriaOrder = [
          { type: 'header', text: '--- ข้อที่ 1: การเขียนย่อความ (10 คะแนน) ---' },
          "s1", "s2", "s3", "s4", "s5", "s6",
          { type: 'summary', part: 1 },
          { type: 'header', text: '--- ข้อที่ 2: การแสดงความคิดเห็น (20 คะแนน) ---' },
          "s7", "s8", "s9", "s10", "s11", "s12", "s13",
          { type: 'summary', part: 2 },
          { type: 'grand_total' }  // 🔥 เพิ่มคะแนนรวมทั้งหมด
        ];

        const criteriaNameMapping = {
          s1: 'ใจความสำคัญ (4 คะแนน)',
          s2: 'การเรียงลำดับและเชื่อมโยงความคิด (2 คะแนน)',
          s3: 'ความถูกต้องตามหลักการเขียนย่อความ (1 คะแนน)',
          s4: 'การสะกดคำ (1 คะแนน)',
          s5: 'การใช้คำ/ถ้อยคำสำนวน (1 คะแนน)',
          s6: 'การใช้ประโยค (1 คะแนน)',
          s7: 'คำบอกข้อคิดเห็น (1 คะแนน)',
          s8: 'เหตุผลสนับสนุน (8 คะแนน)',
          s9: 'การเรียงลำดับและเชื่อมโยงความคิด (3 คะแนน)',
          s10: 'ความถูกต้องตามหลักการแสดงความคิดเห็น (2 คะแนน)',
          s11: 'การสะกดคำ/การใช้ภาษา (2 คะแนน)',
          s12: 'การใช้คำ/ถ้อยคำสำนวน (2 คะแนน)',
          s13: 'การใช้ประโยค (2 คะแนน)',
        };
        ...
        // ---------- แทนที่บล็อกเดิมทั้งหมดด้วยบล็อกนี้ ----------
        criteriaOrder.forEach(item => {
          // object types: header / summary / grand_total
          if (typeof item === 'object') {
            ...
            if (item.type === 'summary') {
              ...
              summaryRow.innerHTML = `
                <td>คะแนนรวมข้อที่ ${item.part}</td>
            ...
            if (item.type === 'grand_total') {
              ...
              grandTotalRow.innerHTML = `
                <td style="text-align: center; background-color: #28a745; color: white;">🏆 คะแนนรวมทั้งหมด (30 คะแนน)</td>
                <td class="score-cell score-ai" style="background-color: #d1ecf1; color: #004085; font-size: 18px;">${totalAI.toFixed(1)}</td>
                <td class="score-cell score-teacher" style="background-color: #d4edda; color: #155724; font-size: 18px;">${totalT1 > 0 ? totalT1.toFixed(1) : '-'}</td>
                <td class="score-cell score-teacher" style="background-color: #d4edda; color: #155724; font-size: 18px;">${totalT2 > 0 ? totalT2.toFixed(1) : '-'}</td>
          ...
          // prepare details text (safe parse)
          let details = "-";
          if (criteriaData.feedback) {
            try {
              const feedbackObj = typeof criteriaData.feedback === 'string' ? JSON.parse(criteriaData.feedback) : criteriaData.feedback;
              details = JSON.stringify(feedbackObj, null, 2);
            } catch (e) {
              details = String(criteriaData.feedback);
            }
          }

          // สร้าง row ที่นี่เสมอ (เพื่อป้องกัน row not defined)
          const row = document.createElement("tr");
          row.innerHTML = `
            <td style="text-align:left">${criteriaName}</td>
